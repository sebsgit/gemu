...
CUresult cuInit (unsigned int flags) {
	if (!_default_cuda_device) {
		_default_cuda_device = new gemu::Device(1024 * 1024);
	}
...
CUresult cuDeviceTotalMem ( size_t* bytes, CUdevice dev ) {
	if (dev == _default_cuda_device_id)
		*bytes = _default_cuda_device->memory()->size();
...
CUresult cuMemcpyHtoD ( CUdeviceptr dstDevice, const void* srcHost, size_t byteCount ) {
	if (_default_cuda_device->memory()->isValid(srcDevice) && (memcpy(dstDevice, srcHost, byteCount) == 0)) {
		return CUDA_SUCCESS_
	}
	return CUDA_ERROR_INVALID_VALUE_;
}
